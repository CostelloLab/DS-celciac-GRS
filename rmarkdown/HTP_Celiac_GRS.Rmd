...
celiac_cv  <-  function( DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp, sampling_iterations = 10, k = 1) {
    ...
    groups = split(celiac_subjects, cut(seq_along(celiac_subjects), group_numbers, labels = F))
    weighted_means <- list()
...
# 50 test_iteations, 50 sampling_iterations
  for(snp in names(DS_dosage)[1:49]){
    ...
    if(!(dim(breakdown)[1] == 2 && any(breakdown == 0))) {
        ...
        grs_results[[snp]] = list(weighted_mean_weight = NA, mean_accuracy = NA)
        print(paste0(round(which(names(DS_dosage)==snp)/49,2)*100,"% complete"))
...
grs_wrapper <- function(DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp, sampling_iterations = 10, k = 1) {
    ...
    rownames(grs_results) <- names(DS_dosage)[1:49]
                                        ...
                                        # create GRS
    GRS <- colSums(apply(DS_dosage[,1:49], 1, function(x) x* as.numeric(grs_results$weighted_mean_weight)), na.rm = TRUE)
  ...
  # plot the roc curves
    ...
    scores <- cbind(sharp_scores, as.numeric(grs_results$weighted_mean_weight))
...
k1 <- grs_wrapper(DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp, sampling_iterations = 10, k = 1)
