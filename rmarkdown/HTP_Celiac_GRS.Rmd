...
grs_wrapper <- function(DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp, sampling_iterations = 10, k = 1) {
    ...
    rownames(grs_results) <- names(DS_dosage)[1:49]
  
                                        # create GRS
    GRS <- colSums(apply(DS_dosage[,1:49], 1, function(x) x* as.numeric(grs_results$weighted_median_weight)), na.rm = TRUE)
  ...
  # plot the roc curves
    ...
    scores <- cbind(sharp_scores, as.numeric(grs_results$weighted_median_weight))
    ...
    p2 <- ggplot(scores, aes(y= value, x=Variant, fill = name)) +
        ...
        theme(axis.text.x = element_text(angle = 90))
                                        ...
                                        # p2
                                        # dev.off()
    accuracies <- sapply(grs_results, function(x) x$accuracies)
...
k1 <- grs_wrapper(DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 5, control_to_case_ratio = 3, snp = snp, sampling_iterations = 5, k = 1)
