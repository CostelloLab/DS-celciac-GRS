...
celiac_log_reg <- function(subjects = subjects, DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, control_to_case_ratio = 3, snp = snp ) {
    ...
    tmp_data <- dosage_data[c(celiac_subjects[-subjects],train_controls),]

    tmp_model <- suppressWarnings(glm(tmp_data$Celiac ~ tmp_data[[snp]],family= binomial(link = "logit")))
...
celiac_auc <- function(subjects = subjects, DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp) {
    ...
    for(x in 1:test_iterations){
        test_controls <- sample(control_subjects[-train_samples],control_to_case_ratio)
        test_data <- dosage_data[c(subjects, test_controls),c(snp,'Celiac')]
        test_data$prediction <- test_data[, snp]*weight
        tmp_auc <- suppressMessages(roc(response = test_data$Celiac, predictor = test_data$prediction)$auc)
    }

    return(list(model = model, auc = median(as.numeric(tmp_auc))))
}
```
```{r control_sampling}
celiac_control_sampling <- function(subjects= subjects, DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp, sampling_iterations = 10) {
    ...
    for(i in 1:sampling_iterations) {
      all_models[[i]] <- celiac_auc(subjects, DS_dosage, control_subjects, celiac_subjects, test_iterations = test_iterations, control_to_case_ratio, snp = snp)
    }
