...
celiac_log_reg <- function(subjects = subjects, DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, control_to_case_ratio = 3, snp = snp, bootstrap = FALSE ) {
    if(bootstrap) {
        train_samples  <-sample(length(control_subjects), (length(celiac_subjects)*control_to_case_ratio))
        train_controls <- control_subjects[train_samples]
        train_cases <- sample(celiac_subjects, length(celiac_subjects)*control_to_case_ratio,replace = TRUE)
        tmp_data <- DS_dosage[c(train_controls, train_cases),]
    } else {
...
celiac_accuracy <- function(subjects = subjects, DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp) {
    ...
    for(x in 1:test_iterations){
        ...
        test_roc <- pROC::roc(response = test_data$Celiac, predictor = test_data$probabilities) # roc curve for the test data
        J <-  test_roc$sensitivities - test_roc$specificities # using youden's J statistic to choose the best threshold. See https://machinelearningmastery.com/threshold-moving-for-imbalanced-classification/
        ix <- max(J[is.finite(test_roc$thresholds)])
        test_data$prediction <- ifelse(test_data$probabilities > test_roc$thresholds[2], 1, 0) # Choosing the threshold for prediction       
...
celiac_control_sampling <- function(subjects= subjects, DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp, sampling_iterations = 10) {
    ...
    weighted_mean_weight <- weighted.mean(weights, accuracies)
