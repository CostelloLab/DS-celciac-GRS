...
celiac_cv  <-  function( DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp, sampling_iterations = 10, k = 1) {
    ...
    for(i in 1:length(groups)) {
        weighted_medians[[i]] <- celiac_control_sampling(groups[[i]], DS_dosage=DS_dosage, control_subjects = control_subjects, celiac_subjects= celiac_subjects, control_to_case_ratio=control_to_case_ratio, snp=snp)
    }
...
celiac_GRS <- function(DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = 10, control_to_case_ratio = 3, snp = snp, sampling_iterations = 10, k = 1) {
  ...
  for(snp in names(DS_dosage)[1:49]){
    ...
    if(!(dim(breakdown)[1] == 2 & any(breakdown == 0))) {
        grs_results[[snp]] <- celiac_cv(DS_dosage = DS_dosage, control_subjects = control_subjects, celiac_subjects = celiac_subjects, test_iterations = test_iterations, control_to_case_ratio = control_to_case_ratio, snp = snp, sampling_iterations = sampling_iteations, k = k)
        print(paste0(round(which(names(dosage_data)==snp)/49,2)*100,"% complete"))


    } else {
        grs_reslt[[snp]] = list(weighted_mean_weight = NA, mean_accuracy = NA)
        print(paste0(round(which(names(dosage_data)==snp)/49,2)*100,"% complete"))
